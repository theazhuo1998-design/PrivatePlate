...
export function ConversationPanel({
  ...
  const logMessages = secondary ? visibleMessages.slice(-2) : visibleMessages;
  ...
  return (
    <section
      className={`chat-panel${secondary ? " chat-panel-secondary" : ""}`}
      aria-label={secondary ? "本餐对话" : "PrivatePlate Agent 对话"}
    >
      {secondary ? (
        <header className="chat-header chat-header-secondary">
          <p className="chat-kicker">对话</p>
          <h2>需要时再细说</h2>
        </header>
      ) : (
      ...
      {secondary ? null : <div className="chat-divider" aria-hidden="true" />}
        ...
        <form className="composer" onSubmit={submit}>
          ...
          <div className="composer-box">
            <textarea
              ...
              title={!operable ? MODEL_OFFLINE_RECOVERY : undefined}
              rows={1}
            />
            <div className="composer-toolbar">
              ...
              <button
                ...
                type="submit"
                disabled={
                  inputDisabled || !input.trim() || selectedDinerIds.length === 0
                }
                aria-label="发送消息"
                title={!operable ? MODEL_OFFLINE_RECOVERY : undefined}
              >
