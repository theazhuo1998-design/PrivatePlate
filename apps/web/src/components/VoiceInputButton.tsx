...
export function VoiceInputButton({
          ...
          : status === "unsupported"
            ...
            : status === "error"
              ...
              : isHold
                ? "按住说话"
                : "可录音";
  const holdBusy = disabled || status === "transcribing";
  const statusVisible =
    showStatus ||
    status === "unsupported" ||
    status === "error" ||
  ...
  function onHoldPointerDown(
    ...
    if (!isHold || holdBusy || isRecording) return;
    event.preventDefault();
    event.currentTarget.setPointerCapture(event.pointerId);
    void startRecording();
  }

  function onHoldPointerUp(): void {
    if (!isHold || !isRecording) return;
    stopRecording();
  }

  return (
    <div className={`voice-input-control ${isHold ? "voice-input-hold" : ""}`}>
      <button
        ...
        onPointerDown={isHold ? onHoldPointerDown : undefined}
        onPointerUp={isHold ? onHoldPointerUp : undefined}
        onPointerCancel={isHold ? onHoldPointerUp : undefined}
        onContextMenu={isHold ? (event) => event.preventDefault() : undefined}
        aria-label={
          isHold
            ? isRecording
              ? "松开停止录音"
      ...
      >
        <SourceIcon name="mic" size={isHold ? 22 : 18} />
