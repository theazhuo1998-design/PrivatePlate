...
export function InventoryImageIntake({
  ...
  variant = "drawer",
  snapshotLabel = "拍冰箱快照",
  sourceLabel = INPUT_SOURCE_FRIDGE_SNAPSHOT,
  disabledReason,
  panelHost = null
  ...
  const imageSupported =
    typeof FileReader !== "undefined" && typeof File !== "undefined";
  ...
  const snapshotTitle =
    !operable
      ? disabledReason ?? MODEL_OFFLINE_RECOVERY
      : undefined;
  const showStatusPanel =
    (hubLayout && !imageSupported) ||
  ...
  const statusPanel = (
    <>
      {hubLayout && !imageSupported ? (
        <p className="intake-unsupported" role="status">
          当前环境不支持拍照识别。
        </p>
      ) : null}
      {imageDataUrl ? (
      ...
      {stage === "recognizing" ? (
        <p className="intake-progress" role="status">正在识别照片…</p>
      ) : null}
      {stage === "error" ? (
  ...
  return (
    <section
      className={`inventory-image-intake${hubLayout ? " intake-hub" : ""}`}
      aria-labelledby="inventory-image-title"
    >
      {hubLayout ? (
        <button
          className="hub-snapshot-button"
          type="button"
          disabled={snapshotDisabled}
          title={snapshotTitle ?? sourceLabel}
          onClick={chooseImage}
        >
          <SourceIcon name="camera" size={18} />
        ...
        <div className="intake-section-heading">
          <div>
            ...
            <p className="input-source-label">{sourceLabel}</p>
          </div>
          <button
      ...
      {hubLayout ? (
        <h3 id="inventory-image-title" className="visually-hidden">
          {snapshotLabel}
        </h3>
      ) : null}
      <input
      ...
      />
      {hubLayout
        ? showStatusPanel && panelHost
          ? createPortal(
              <div className="intake-hub-panel">{statusPanel}</div>,
              panelHost
            )
          : null
        : statusPanel}
