...
export function MealStatusBar({
  ...
  // Only show after plan exists; emphasize after task save or when ready to log meal.
  if (!taskSaved && actionStatus !== "plan" && actionStatus !== "pending") {
    return null;
  }
      ...
      <div className="meal-status-body">
        ...
        <div className="meal-status-actions">
          {stage === "ready_to_eat" ? (
            ...
            <p className="meal-status-hint">
              请先保存任务卡/采购清单；保存后这里会出现「确认吃完」按钮。
            </p>
          ) : (
            <>
              <p className="meal-status-hint">
                可继续改菜单，或说「预览任务卡」生成采购/任务。若已自己做完，也可直接确认吃完。
              </p>
              <button
                type="button"
