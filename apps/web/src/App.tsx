...
import { CAPABILITY_LINE, taskSaveMessage } from "./task-board";
...
export function App() {
  ...
  const refreshShell = useCallback(async () => {
    ...
    setInbox(nextInbox);
    setDayContext(nextDay);
    const refreshed = { context: nextContext, dayContext: nextDay };
    ...
    if (
      ...
      const target = confirmationDrawerTarget(savedConfirmation);
      if (target !== "plan") setDrawerTarget(target);
  ...
  function selectNav(target: NavTarget): void {
    if (target === "new") {
      setDrawerTarget(null);
      void onReset();
      return;
    ...
    try {
      ...
      await runAgentStream({
        ...
        onEvent: (event: AgentEvent) => {
          ...
          if (event.type === "plan_infeasible") {
            setPlanInfeasible(true);
            setActionStatus("idle");
          }
      ...
      const systemText =
        actionType === "inventory_restock"
          ...
          : actionType === "member_memory_change"
            ...
            : replayed
              ? "这份安排已经确认过了。"
              : `已确认本餐安排，${taskSaveMessage(runtime?.appMode)}。${CAPABILITY_LINE}。`;
      setMessages((previous) => [
        ...previous,
      ...
      setMessages([
        {
          id: "reset",
          role: "assistant",
          text: "新对话已开始。家庭记忆、库存和账本仍保留，我们可以重新安排这一餐。"
        }
      ]);
      await refreshShell();
    } catch (requestError) {
  ...
  return (
    <div
      className={`app product-app ${drawerTarget ? "drawer-open" : ""}`}
      aria-busy={busy || shellLoading}
    >
      <Sidebar
