...
export function validateFinalAnswer(input: ValidationInput): ValidationResult {
  ...
  // Structured action status is the single source of truth for save/send claims.
  // Success wording is only valid when confirmationStatus === "committed".
  // After commit, UI success copy is local/household save — not 已发送.
  const claimsSent =
    /已发送|已保存到收件箱|发送成功|已经交给保姆执行完毕|已保存到本地演示数据库|已保存到当前家庭空间|消息已送达|已发送给保姆|收件人已收到/.test(
      input.answer
    );
