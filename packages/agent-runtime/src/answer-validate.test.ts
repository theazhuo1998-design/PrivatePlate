...
describe("final answer write-claim validation", () => {
  ...
  it("rejects unconfirmed send or local-save wording", () => {
    for (const answer of [
      "任务已发送。",
      "已保存到本地演示数据库。",
      "已保存到当前家庭空间。"
      ...
      expect(
        ...
        ok: false,
        reasons: ["send_wording_without_commit"]
      });
    }
  });

  it("allows local-save wording only after commit, and does not require 已发送", () => {
    expect(
      validateFinalAnswer(
        input({
          answer: "已保存到本地演示数据库。",
