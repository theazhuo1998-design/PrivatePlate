...
export const NUTRITION_BUDGET_BOUNDARY =
  "克级建议区间；确定性、可版本化的营养计算；方向性合理的工程估算；不是医疗诊断、治疗建议或个体化处方；结果仍需结合医生或营养师建议。";
export const NUTRITION_BUDGET_FALLBACK_BOUNDARY = `成员资料不足，使用演示默认值。${NUTRITION_BUDGET_BOUNDARY}`;
...
export function calculateMemberNutritionBudget(
  ...
  if (!profile) {
    ...
    return {
      ...
      confidence: "low",
      reasonCodes: ["missing_profile_fallback", "fallback_demo_estimate"],
      applicability: {
        fullySupportedMealTypes: ["lunch", "dinner"],
        extensionMealSlots: ["breakfast", "snack"],
        boundary: NUTRITION_BUDGET_FALLBACK_BOUNDARY
      }
    };
  }
  ...
  return {
    ...
    applicability: {
      fullySupportedMealTypes: ["lunch", "dinner"],
      extensionMealSlots: ["breakfast", "snack"],
      boundary: NUTRITION_BUDGET_BOUNDARY
    }
  };
}
