...
import { enrichMemberNutritionBudget, toSharedHouseholdDisplay } from "../nutrition-display.js";
import {
...
function loadMealPolicies(db: Db, memberIds: string[]): MemberMealPolicy[] {
  if (memberIds.length === 0) return [];
  const rows = db
    .prepare(
      `SELECT member_id, meal_type, portion_units_json, guardrails_json, rule_version
       FROM member_meal_policies
       WHERE member_id IN (${memberIds.map(() => "?").join(",")})`
    )
    .all(...memberIds) as Array<{
    member_id: string;
    meal_type: MemberMealPolicy["mealType"];
  ...
  return rows.map((row) => ({
    ...
    portionUnitsByRole: JSON.parse(row.portion_units_json),
...
export function getDayContext(
  ...
  const focusedMembers = members.filter((m) => focusIds.has(m.id));
  const mealPolicies = loadMealPolicies(
    db,
    focusedMembers.map((m) => m.id)
  );

  const memberViews = focusedMembers
    .map((member) => {
      ...
      const hardConstraints = hardConstraintRows.map((c) => ({
        id: c.id,
        kind: c.kind,
      ...
      const nutritionProfile: MemberNutritionProfile | undefined = profileRow
        ...
        : undefined;
      const nutritionBudget = enrichMemberNutritionBudget({
        budget: calculateMemberNutritionBudget(nutritionProfile, member.id),
        mealPolicies: mealPolicies.filter((policy) => policy.memberId === member.id),
        hardConstraints,
        preferenceKinds: preferences.map((p) => p.kind),
        ...(nutritionProfile ? { weightGoal: nutritionProfile.weightGoal } : {})
      });
  ...
  return {
    ...context,
    sharedDisplay: toSharedHouseholdDisplay(context)
  };
}
