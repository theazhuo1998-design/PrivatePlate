...
import { toSharedHouseholdDisplay } from "../nutrition-display.js";
...
export class PrivatePlateDomain {
  ...
  /** Sanitized household nutrition projection for the shared kitchen display. */
  getSharedHouseholdDisplay(input?: {
    serviceDate?: string;
    timeZone?: string;
    dinerIds?: string[];
    plan?: MealPlan;
  }) {
    const day = this.getDayContext(input);
    if (!input?.plan) return day.sharedDisplay;
    return toSharedHouseholdDisplay(day, { plan: input.plan });
  }
